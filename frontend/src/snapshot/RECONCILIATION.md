...
1. The frontend **renders only** the frozen top-level sections:
   ...
   plus the optional `details`. `src/snapshot/schema.js` enforces this
   whitelist and fails loudly on unknown top-level sections, so a future #114
   payload cannot silently introduce undocumented surfaces.
