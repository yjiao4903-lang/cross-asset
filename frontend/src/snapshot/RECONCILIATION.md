...
| `clusters[]` (`cluster_id/family/horizon/score/weekly_delta/direction/confidence/coverage/top_positive/top_negative/missing_factors/stale_factors/freshness_status`) | heatmap (family × horizon), Weekly Pulse delta chart, drill-down | `freshness_status` vocabulary map: `OK→FRESH`, `PARTIAL→PARTIAL` (amber warning, kept distinct from STALE per R3), `STALE→STALE`, others pass through |
