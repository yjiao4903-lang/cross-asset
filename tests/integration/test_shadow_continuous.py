...
def test_shadow_seven_dates_failure_stale_and_weekly(tmp_path):
    ...
    assert len(outputs)==7 and outputs[2]['status']=='DEGRADED'
    # A single observation is insufficient for the 1M trend signal, so the
    # safer contract is to freeze every allocation rather than emit an ACTIVE
    # recommendation from missing critical inputs.
    assert all(x['allocation_status']=='FROZEN' for x in outputs)
    weekly=r.weekly_report(dates); assert weekly['run_count']==7 and weekly['degraded_count']==1 and weekly['frozen_count']==7
