...
def test_future_available_at_is_invisible():
    ...
    cutoff = datetime(2026, 9, 4, 16, 0, tzinfo=BEIJING)
    assert latest_on_or_before(rows, "US_EQ", date(2026, 9, 4), cutoff) is None
...
def test_missing_lookback_is_partial_not_zero():
    rows = [
    ...
    assert equity["changes"]["1w"]["value"] is None
...
def test_compare_weeks_and_cli_fixture(tmp_path: Path):
    ...
    assert compare_weeks(current, prior)[0]["value"] == 0.1
    ...
    assert result["admission"] == "DEVELOPMENT_PRIOR"
    assert "+08:00" in result["data_cutoff"]
    assert result["requested_cutoff"]
    assert result["source_cutoff"]
    assert result["effective_cutoff"]
