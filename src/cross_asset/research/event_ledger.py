...
def evaluate_claim_record(claim: ResearchClaim, *, now: Any, observation: EventObservation | None) -> ClaimEvaluation:
    ...
    return ClaimEvaluation(
        ...
        result.status,
        _iso(now) or "",
        (observation.source,) if is_outcome else (),
        observation.target if is_outcome else None,
        observation.as_of if is_outcome else None,
        observation.value if is_outcome else None,
        observation.unit if is_outcome else None,
        observation.available_at if is_outcome else None,
        observation.revision if is_outcome else None,
    )
